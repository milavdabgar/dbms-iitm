...
\begin{frame}{Cardinality Constraints on Ternary Relationship}
    \begin{itemize}
        ...
        \begin{itemize}
            \item a) Each $A$ entity is associated with a unique entity from $B$ and $C$ or
            \item b) Each pair of entities from $(A, B)$ is associated with a unique $C$ entity, and each pair $(A, C)$ is associated with a unique $B$
        \end{itemize}
...
\begin{frame}{Design Constraints on a Specialization / Generalization}
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{total}: an entity must belong to one of the lower-level entity sets
            \item \textbf{partial}: an entity need not belong to one of the lower-level entity sets
        \end{itemize}
...
\begin{frame}{Aggregation (2)}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Every \texttt{eval\_for} relationship corresponds to a \texttt{proj\_guide} relationship
            \item However, some \texttt{proj\_guide} relationships may not correspond to any \texttt{eval\_for} relationships
            \item $\triangleright$ So we cannot discard the \texttt{proj\_guide} relationship
        \end{itemize}
...
\begin{frame}{Representing Aggregation via Schema}
    \begin{itemize}
        ...
        \item In our example:
        \begin{itemize}
            \item The schema \texttt{eval\_for} is: \texttt{eval\_for (s\_ID, project\_id, i\_ID, evaluation\_id)}
...
\begin{frame}{Entities vs Relationship Sets}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Possible guideline is to designate a relationship set to describe an action that occurs between entities
        \end{itemize}
    \end{itemize}
    \begin{center}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item For example, attribute \texttt{date} as attribute of \texttt{advisor} or as attribute of \texttt{student}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Binary vs Non-Binary Relationships}
    \begin{itemize}
        ...
        \begin{itemize}
            \item For example, a ternary relationship \texttt{parents}, relating a child to his/her father and mother, is best replaced by two binary relationships, \texttt{father} and \texttt{mother}
...
\begin{frame}{Binary vs Non-Binary Relationships (2): Conversion}
    \begin{itemize}
        ...
        \begin{itemize}
            \item a) a new entity $e_i$ in the entity set $E$
            \item b) add $(e_i, a_i)$ to $R_A$
            \item c) add $(e_i, b_i)$ to $R_B$
            \item d) add $(e_i, c_i)$ to $R_C$
        \end{itemize}
    \end{itemize}
\end{frame}
