...
\begin{frame}{Library Information System (LIS)}
    ...
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] Identify the Entity Sets with attributes
            \item[$\triangleright$] Identify the Relationships
            \item[$\triangleright$] Build the initial set of relational schema
            \item[$\triangleright$] Refine the set of schema with FDs that hold on them
            \item[$\triangleright$] Finalize the design of the schema
        \end{itemize}
...
\begin{frame}{LIS Specs Excerpts}
    \begin{itemize}[<+->]
        ...
        \item Every book has
        \begin{itemize}
            \item[$\triangleright$] title
            \item[$\triangleright$] author (in case of multiple authors, only the first author is maintained)
            \item[$\triangleright$] publisher
            \item[$\triangleright$] year of publication
            \item[$\triangleright$] ISBN number (which is unique for the publication), and
            \item[$\triangleright$] accession number (which is the unique number of the copy of the book in the library)
        \end{itemize}
...
\begin{frame}{LIS Specs Excerpts (2)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] undergraduate students
            \item[$\triangleright$] post graduate students
            \item[$\triangleright$] research scholars, and
            \item[$\triangleright$] faculty members
        \end{itemize}
...
\begin{frame}{LIS Specs Excerpts (4)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] A book may be issued to a member if it is not already issued to someone else (trivial)
            \item[$\triangleright$] A book may not be issued to a member if another copy of the same book is already issued to the same member
            \item[$\triangleright$] No issue will be done to a member if at the time of issue one or more of the books issued by the member has already exceeded its duration of issue
            \item[$\triangleright$] No issue will be allowed also if the quota is exceeded for the member
        \end{itemize}
...
\begin{frame}{LIS Schema Refinement (13): Final}
    \begin{exampleblock}{Final Schema}
        \begin{itemize}[<+->]
            \item \texttt{book\_catalogue (title, author\_fname, author\_lname, publisher, year, ISBN\_no)}
            \item \texttt{book\_copies (ISBN\_no, accession\_no)}
            \item \texttt{book\_issue (member\_no, accession\_no, doi)}
