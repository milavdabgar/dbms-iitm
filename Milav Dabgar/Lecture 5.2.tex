...
\begin{frame}{Goal: Devise a Theory for Good Relations}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] each relation is in good form
            \item[$\triangleright$] the decomposition is a lossless-join decomposition
        \end{itemize}
        \item The theory is based on:
        \begin{itemize}
            \item[$\triangleright$] Functional dependencies
            \item[$\triangleright$] Multivalued dependencies
            \item[$\triangleright$] Other dependencies
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Functional Dependencies (3)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] $K \rightarrow R$ and
            \item[$\triangleright$] for no $\alpha \subset K$, $\alpha \rightarrow R$
        \end{itemize}
...
\begin{frame}{Functional Dependencies (4)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] test relations to see if they are legal under a given set of functional dependencies.
            \item[$\triangleright$] If a relation $r$ is legal under a set $F$ of functional dependencies, we say that $r$ satisfies $F$
            \item[$\triangleright$] specify constraints on the set of legal relations
        ...
        \begin{itemize}
            \item[$\triangleright$] For example, a specific instance of \texttt{instructor} may, by chance, satisfy \texttt{name $\rightarrow$ ID}
            \item[$\triangleright$] In such cases we do not say that $F$ holds on $R$
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Functional Dependencies (6)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] \texttt{StudentID $\rightarrow$ Semester}
            \item[$\triangleright$] \texttt{StudentID, Lecture $\rightarrow$ TA}
            \item[$\triangleright$] \texttt{\{StudentID, Lecture\} $\rightarrow$ \{TA, Semester\}}
        \end{itemize}
    \end{itemize}
    \begin{center}
...
\begin{frame}{Functional Dependencies (7)}
    \begin{itemize}[<+->]
        \item Functional dependencies are:
        \begin{itemize}
            \item[$\triangleright$] \texttt{EmployeeID $\rightarrow$ EmployeeName}
            \item[$\triangleright$] \texttt{EmployeeID $\rightarrow$ DepartmentID}
            \item[$\triangleright$] \texttt{DepartmentID $\rightarrow$ DepartmentName}
        \end{itemize}
    \end{itemize}
    \begin{center}
    \begin{tabular}{|l|l|l|l|}
