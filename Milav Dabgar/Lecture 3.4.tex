...
\begin{frame}{Transactions}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] either fully executed or rolled back as if it never occurred
            \end{itemize}
            ...
            \begin{itemize}
                \item[$\triangleright$] Not supported on most databases
            \end{itemize}
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}[fragile]{Not Null and Unique Constraints}
    \begin{block}{Usage}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] Declare name and budget to be \texttt{not null}
...
budget numeric(12,2) not null
            ...
            \begin{itemize}
                \item[$\triangleright$] The \texttt{unique} specification states that the attributes $A_1, A_2, ..., A_m$ form a candidate key
                \item[$\triangleright$] Candidate keys are permitted to be null (in contrast to primary keys).
            \end{itemize}
        \end{itemize}
    \end{block}
\end{frame}
...
create table person ( 
    ...
    foreign key mother references person
            ...
            \begin{itemize}
                \item[$\triangleright$] Insert father and mother of a person before inserting person
                \item[$\triangleright$] OR, Set father and mother to null initially, update after inserting all persons (not possible if father and mother attributes declared to be \texttt{not null})
                \item[$\triangleright$] OR Defer constraint checking (will discuss later)
            \end{itemize}
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Built-in Data Types in SQL}
    \begin{block}{Data Types}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] Example: \texttt{date '2005-7-27'}
            \end{itemize}
            \item \texttt{time}: Time of day, in hours, minutes and seconds.
            \begin{itemize}
                \item[$\triangleright$] Example: \texttt{time '09:00:30'}, \texttt{time '09:00:30.75'}
            \end{itemize}
            \item \texttt{timestamp}: date plus time of day
            \begin{itemize}
                \item[$\triangleright$] Example: \texttt{timestamp '2005-7-27 09:00:30.75'}
            \end{itemize}
            \item \texttt{interval}: period of time
            \begin{itemize}
                \item[$\triangleright$] Example: \texttt{interval '1' day}
            \end{itemize}
...
\begin{frame}{Large-Object Types}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] \textbf{\texttt{blob}}: binary large object - object is a large collection of uninterpreted binary data (whose interpretation is left to an application outside of the database system)
                \item[$\triangleright$] \textbf{\texttt{clob}}: character large object - object is a large collection of character data
            \end{itemize}
...
\begin{frame}{Authorization}
    \begin{block}{Forms of Authorization}
        \begin{itemize}[<+->]
            \item Forms of authorization on parts of the database:
            \begin{itemize}
                \item[$\triangleright$] \textbf{Read} - allows reading, but not modification of data
                \item[$\triangleright$] \textbf{Insert} - allows insertion of new data, but not modification of existing data
                \item[$\triangleright$] \textbf{Update} - allows modification, but not deletion of data
                \item[$\triangleright$] \textbf{Delete} - allows deletion of data
            \end{itemize}
            \item Forms of authorization to modify the database schema
            \begin{itemize}
                \item[$\triangleright$] \textbf{Index} - allows creation and deletion of indices
                \item[$\triangleright$] \textbf{Resources} - allows creation of new relations
                \item[$\triangleright$] \textbf{Alteration} - allows addition or deletion of attributes in a relation
                \item[$\triangleright$] \textbf{Drop} - allows deletion of relations
            \end{itemize}
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Authorization Specification in SQL}
    \begin{block}{Grant Statement}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] a user-id
                \item[$\triangleright$] \texttt{public}, which allows all valid users the privilege granted
                \item[$\triangleright$] A \textbf{role} (more on this later)
            \end{itemize}
...
\begin{frame}[fragile]{Privileges in SQL}
    \begin{block}{Privileges List}
        \begin{itemize}[<+->]
            \item \textbf{\texttt{select}}: allows read access to relation, or the ability to query using the view
            \begin{itemize}
                \item[$\triangleright$] Example: grant users $U_1, U_2,$ and $U_3$ select authorization on the \texttt{instructor} relation:
                \begin{lstlisting}[language=SQL]
...
select * from geo_instructor;
            ...
            \item What if
            \begin{itemize}
                \item[$\triangleright$] \texttt{geo\_staff} does not have permissions on \texttt{instructor}?
                \item[$\triangleright$] creator of view did not have some permissions on \texttt{instructor}?
            \end{itemize}
        \end{itemize}
    \end{exampleblock}
\end{frame}
