...
\begin{frame}{Functional Dependencies: Armstrong's Axioms}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{Complete} (eventually generate all functional dependencies that hold)
        \end{itemize}
...
\begin{frame}{Functional Dependencies (5): Armstrong's Axioms: Derived Rules}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{Decomposition}: if $\alpha \rightarrow \beta\gamma$ holds, then $\alpha \rightarrow \beta$ holds and $\alpha \rightarrow \gamma$ holds
...
\begin{frame}{Functional Dependencies (7): Closure of Attribute Sets: Example}
    \begin{itemize}
        ...
        \begin{itemize}
            \item a) result = $AG$
            \item b) result = $ABCG$ ($A \rightarrow C$ and $A \rightarrow B$)
            \item c) result = $ABCGH$ ($CG \rightarrow H$ and $CG \subseteq AGBC$)
            \item d) result = $ABCGHI$ ($CG \rightarrow I$ and $CG \subseteq AGBCH$)
        \end{itemize}
...
\begin{frame}{BCNF (2): Decomposition}
    \begin{itemize}
        ...
        \item In our example,
        \begin{itemize}
            \item $\alpha = \text{dept\_name}$
            \item $\beta = \text{building, budget}$
            \item $\text{dept\_name} \rightarrow \text{building, budget}$
        \end{itemize}
        \item \texttt{inst\_dept} is replaced by
        \begin{itemize}
            \item $(\alpha \cup \beta) = (\text{dept\_name, building, budget})$
            \item $\triangleright \text{dept\_name} \rightarrow \text{building, budget}$
            \item $(R - (\beta - \alpha)) = (\text{ID, name, salary, dept\_name})$
            \item $\triangleright \text{ID} \rightarrow \text{name, salary, dept\_name}$
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{BCNF (3): Lossless Join}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Decomposition is lossy if $R_1 \bowtie R_2 \supset R$
            \item Decomposition is lossless if $R_1 \bowtie R_2 = R$
        \end{itemize}
        \item To check for lossless join decomposition using FD set, following must hold:
        \begin{itemize}
            \item Union of Attributes of $R_1$ and $R_2$ must be equal to attribute of $R$
            \[ R_1 \cup R_2 = R \]
            \item Intersection of Attributes of $R_1$ and $R_2$ must not be NULL
            \[ R_1 \cap R_2 \neq \phi \]
            \item Common attribute must be a key for at least one relation ($R_1$ or $R_2$)
...
\begin{frame}{BCNF (4): Dependency Preservation}
    \begin{itemize}
        ...
        \begin{itemize}
            \item $R = CSZ$, $F = \{CS \rightarrow Z, Z \rightarrow C\}$
            \item Key = $CS$
            \item $CS \rightarrow Z$ satisfies BCNF, but $Z \rightarrow C$ violates
        \end{itemize}
        \item Decompose as: $R_1 = ZC$, $R_2 = CSZ - (C - Z) = SZ$
        \begin{itemize}
            \item $R_1 \cup R_2 = CSZ = R$, $R_1 \cap R_2 = Z \neq \phi$, and $R_1 \cap R_2 = Z \rightarrow ZC = R_1$. So it has lossless join
        \end{itemize}
...
\begin{frame}{3NF: Third Normal Form}
    \begin{itemize}
        ...
        \begin{itemize}
            \item $\alpha \rightarrow \beta$ is trivial (that is, $\beta \subseteq \alpha$)
            \item $\alpha$ is a superkey for $R$
            \item Each attribute A in $\beta - \alpha$ is contained in a candidate key for $R$ (Note: Each attribute may be in a different candidate key)
        \end{itemize}
...
\begin{frame}{Problems with Decomposition}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item What is the building for an instructor?
        \end{itemize}
    \end{itemize}
    \vspace{1em}
