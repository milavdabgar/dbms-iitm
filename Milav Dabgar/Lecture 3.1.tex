...
where course_id like '___-%';
    ...
    \end{center}
    \begin{itemize}
        \item \small The course id of each department has either 2 or 3 alphabets in the beginning, followed by a hyphen and then followed by a 3-digit number. The above query returns the names of those departments that have 3 alphabets in the beginning.
    \end{itemize}
\end{frame}
...
\begin{frame}{Module Summary}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Select
            \item Cartesian Product / \texttt{as}
            \item Where: \texttt{and} / \texttt{or}
            \item String Matching
            \item \texttt{Order by}
            \item \texttt{in}
            \item Set Operations: \texttt{union}, \texttt{intersect}, \texttt{except}
            \item Aggregate Functions: \texttt{avg}, \texttt{min}, \texttt{max}, \texttt{count}, \texttt{sum}
        \end{itemize}
    \end{itemize}
    \vspace{2em}
    \begin{center}
