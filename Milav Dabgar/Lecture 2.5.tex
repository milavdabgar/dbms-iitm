...
\begin{frame}{Set Operations (3)}
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{$m + n$} times in \texttt{$r$ union all $s$}
            \item \textbf{$\min(m, n)$} times in \texttt{$r$ intersect all $s$}
            \item \textbf{$\max(0, m - n)$} times in \texttt{$r$ except all $s$}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Null Values (2): Three Valued Logic}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Example: $\texttt{5 < null}$ \quad or \quad $\texttt{null <> null}$ \quad or \quad $\texttt{null = null}$
        \end{itemize}
        \item Three-valued logic using the value \textbf{unknown}:
        \begin{itemize}
            \item \textbf{OR}: \texttt{(unknown or true) = true}, \texttt{(unknown or false) = unknown}, \texttt{(unknown or unknown) = unknown}
            \item \textbf{AND}: \texttt{(true and unknown) = unknown}, \texttt{(false and unknown) = false}, \texttt{(unknown and unknown) = unknown}
            \item \textbf{NOT}: \texttt{(not unknown) = unknown}
        \end{itemize}
...
from instructor;
        ...
        \begin{itemize}
            \item \texttt{count} returns \texttt{0}
            \item all other aggregates return \texttt{null}
        \end{itemize}
    \end{itemize}
\end{frame}
