...
\begin{frame}{Design Goals}
    \begin{block}{Goals for Relational Database Design}
        \begin{itemize}[<+->]
            \item Goal for a relational database design is:
            \begin{itemize}
                \item[$\triangleright$] BCNF / 4NF
                \item[$\triangleright$] Lossless join
                \item[$\triangleright$] Dependency preservation
            \end{itemize}
            \item If we cannot achieve this, we accept one of
            \begin{itemize}
                \item[$\triangleright$] Lack of dependency preservation
                \item[$\triangleright$] Redundancy due to use of 3NF
            \end{itemize}
        \end{itemize}
    \end{block}
    \begin{itemize}[<+->]
...
\begin{frame}{Further Normal Forms}
    \begin{itemize}[<+->]
        \item Further NFs
        \begin{itemize}
            \item[$\triangleright$] Elementary Key Normal Form (EKNF)
            \item[$\triangleright$] Essential Tuple Normal Form (ETNF)
            \item[$\triangleright$] Join Dependencies And Fifth Normal Form (5 NF)
            \item[$\triangleright$] Sixth Normal Form (6NF)
            \item[$\triangleright$] Domain/Key Normal Form (DKNF)
        \end{itemize}
        \item Join dependencies generalize multivalued dependencies
        \begin{itemize}
            \item[$\triangleright$] lead to project-join normal form (PJNF) (also called fifth normal form)
        \end{itemize}
        ...
        \begin{itemize}
            \item[$\triangleright$] Hence rarely used
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Overall Database Design Process}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] $R$ could have been generated when converting E-R diagram to a set of tables
            \item[$\triangleright$] $R$ could have been a single relation containing all attributes that are of interest (universal relation)
        \end{itemize}
...
\begin{frame}{ER Model and Normalization}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] Good design would have made \texttt{department} an entity
        \end{itemize}
...
\begin{frame}{Denormalization for Performance}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] \texttt{Course(course\_id, title, ...)}
            \item[$\triangleright$] \texttt{Prerequisite(course\_id, prereq)}
        \end{itemize}
        ...
        \begin{itemize}
            \item[$\triangleright$] faster lookup
            \item[$\triangleright$] extra space and extra execution time for updates
            \item[$\triangleright$] extra coding work for programmer and possibility of error in extra code
        \end{itemize}
        \item \textbf{Alternative 2:} Use a materialized view defined as \texttt{Course $\bowtie$ Prerequisite}
        \begin{itemize}
            \item[$\triangleright$] Benefits and drawbacks same as above, except no extra coding work for programmer and avoids possible errors
        \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}{Other Design Issues}
    \begin{alertblock}{Bad Database Design Patterns}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] \texttt{earnings\_2004}, \texttt{earnings\_2005}, \texttt{earnings\_2006}, etc., all on the schema \texttt{(company\_id, earnings)}.
...
\begin{frame}{Temporal Databases}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] Medical Records
            \item[$\triangleright$] Judicial records
            \item[$\triangleright$] Share prices
            \item[$\triangleright$] Exchange rates
            \item[$\triangleright$] Interest rates
            \item[$\triangleright$] Company profits
        ...
        \begin{itemize}
            \item[$\triangleright$] Give me last month's history of the Dollar-Pound Sterling exchange rate.
            \item[$\triangleright$] Give me the share prices of the NYSE on October 17, 1996.
        \end{itemize}
...
\begin{frame}{Temporal Data}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] For example, \texttt{course(course\_id, course\_title)} is replaced by \texttt{course(course\_id, course\_title, start, end)}
        \end{itemize}
...
\begin{frame}{Temporal Database Theory}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] Discrete or dense
            \item[$\triangleright$] Bounded or unbounded
            \item[$\triangleright$] Single dimensional or multi-dimensional
            \item[$\triangleright$] Linear or non-linear
            \item[$\triangleright$] Timestamp Model
        \end{itemize}
        \item Temporal ER model by adding valid time to
        \begin{itemize}
            \item[$\triangleright$] Attributes: address of an instructor at different points in time
            \item[$\triangleright$] Entities: time duration when a student entity exists
            \item[$\triangleright$] Relationships: time during which a student attended a course
            \item[$\triangleright$] \textit{But no accepted standard}
        \end{itemize}
...
\begin{frame}{Modeling Temporal Data: Uni / Bi Temporal}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] \textbf{Valid Time}: Time period during which a fact is true in real world, provided to the system.
            \item[$\triangleright$] \textbf{Transaction Time}: Time period during which a fact is stored in the database, based on transaction serialization order and is the timestamp generated automatically by the system.
        \end{itemize}
        ...
        \begin{itemize}
            \item[$\triangleright$] \textbf{Uni-Temporal Relations}: Has one axis of time, either Valid Time or Transaction Time.
            \item[$\triangleright$] \textbf{Bi-Temporal Relations}: Has both axis of time Valid time and Transaction time. It includes Valid Start Time, Valid End Time, Transaction Start Time, Transaction End Time.
        \end{itemize}
    \end{itemize}
\end{frame}
