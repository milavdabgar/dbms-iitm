...
\begin{frame}{MVD: Multivalued Dependency}
    ...
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] Man $\twoheadrightarrow$ Phones
            \item[$\triangleright$] Man $\twoheadrightarrow$ Dogs\_Like
        \end{itemize}
...
\begin{frame}{Post 1NF Normalization}
    ...
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] \texttt{Student(SID, Sname)} where \texttt{SID $\rightarrow$ Sname}
            \item[$\triangleright$] \texttt{Course(CID, Cname)} where \texttt{CID $\rightarrow$ Cname}
        \end{itemize}
...
\begin{frame}{MVD (3)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] \texttt{inst\_child(ID, child\_name)}
            \item[$\triangleright$] \texttt{inst\_phone(ID, phone\_number)}
        \end{itemize}
        \item If we were to combine these schema to get
        \begin{itemize}
            \item[$\triangleright$] \texttt{inst\_info(ID, child\_name, phone\_number)}
        \end{itemize}
        \item Example data:
        \begin{itemize}
            \item[$\triangleright$] \texttt{(99999, David, 512-555-1234)}
            \item[$\triangleright$] \texttt{(99999, David, 512-555-4321)}
            \item[$\triangleright$] \texttt{(99999, William, 512-555-1234)}
            \item[$\triangleright$] \texttt{(99999, William, 512-555-4321)}
        \end{itemize}
...
\begin{frame}{MVD: Example}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] $< y_1, z_1, w_1 > \in r$ and $< y_1, z_2, w_2 > \in r$ then
            \item[$\triangleright$] $< y_1, z_1, w_2 > \in r$ and $< y_1, z_2, w_1 > \in r$
        \end{itemize}
...
\begin{frame}{MVD: Example (2)}
    \begin{itemize}[<+->]
        \item In our example:
        \begin{itemize}
            \item[$\triangleright$] \texttt{ID $\twoheadrightarrow$ child\_name}
            \item[$\triangleright$] \texttt{ID $\twoheadrightarrow$ phone\_number}
        \end{itemize}
        ...
        \item \textbf{Note:}
        \begin{itemize}
            \item[$\triangleright$] If $Y \rightarrow Z$ then $Y \twoheadrightarrow Z$
            \item[$\triangleright$] Indeed we have (in above notation) $Z_1 = Z_2$. The claim follows.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{MVD: Use}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] a) To test relations to determine whether they are legal under a given set of functional and multivalued dependencies
            \item[$\triangleright$] b) To specify constraints on the set of legal relations. We shall thus concern ourselves only with relations that satisfy a given set of functional and multivalued dependencies.
        \end{itemize}
...
\begin{frame}{MVD: Theory}
    ...
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] $Y \subseteq X$ or
            \item[$\triangleright$] $X \cup Y = R$.
        \end{itemize}
...
\begin{frame}{MVD: Theory (2)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] If $\alpha \rightarrow \beta$, then $\alpha \twoheadrightarrow \beta$
        \end{itemize}
...
\begin{frame}{Fourth Normal Form}
    \begin{block}{Definition of 4NF}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] $\alpha \twoheadrightarrow \beta$ is trivial (that is, $\beta \subseteq \alpha$ or $\alpha \cup \beta = R$)
                \item[$\triangleright$] $\alpha$ is a superkey for schema $R$
            \end{itemize}
...
\begin{frame}{Restriction of Multivalued Dependencies}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] All functional dependencies in $D^+$ that include only attributes of $R_i$
            \item[$\triangleright$] All multivalued dependencies of the form
            \begin{itemize}
...
\begin{frame}{4NF Decomposition Algorithm}
    \begin{exampleblock}{Algorithm}
        \begin{itemize}[<+->]
            ...
            \item b) If not, then
            \begin{itemize}
                \item[$\triangleright$] Choose a dependency in $D^+$ that breaks the 4NF rules, say $A \twoheadrightarrow B$
                \item[$\triangleright$] Create $R_1 = AB$
                \item[$\triangleright$] Create $R_2 = (R - (B - A))$
                \item[$\triangleright$] Note that: $R_1 \cap R_2 = A$ and $A \twoheadrightarrow AB \, (= R_1)$, so this is lossless decomposition
            \end{itemize}
            \item c) Repeat for $R_1$, and $R_2$
            \begin{itemize}
                \item[$\triangleright$] By defining $D_1^+$ to be all dependencies in $D$ that contain only attributes in $R_1$
                \item[$\triangleright$] Similarly $D_2^+$
            \end{itemize}
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Example of 4NF Decomposition (3)}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item[$\triangleright$] a) $R_1 = (A, B)$ ($R_1$ is in 4NF)
            \item[$\triangleright$] b) $R_2 = (A, C, G, H, I)$ ($R_2$ is not in 4NF, decompose into $R_3$ and $R_4$)
            \item[$\triangleright$] c) $R_3 = (C, G, H)$ ($R_3$ is in 4NF)
            \item[$\triangleright$] d) $R_4 = (A, C, G, I)$ ($R_4$ is not in 4NF, decompose into $R_5$ and $R_6$)
            \begin{itemize}
            ...
            \end{itemize}
            \item[$\triangleright$] e) $R_5 = (A, I)$ ($R_5$ is in 4NF)
            \item[$\triangleright$] f) $R_6 = (A, C, G)$ ($R_6$ is in 4NF)
        \end{itemize}
    \end{itemize}
\end{frame}
