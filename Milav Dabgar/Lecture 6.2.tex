...
\begin{frame}{3NF Decomposition (2): 3NF Definition}
    \begin{itemize}
        ...
        \begin{itemize}
            \item $A \subseteq X$ (that is, the FD is trivial) or
            \item $X$ is a superkey of $R$ or
            \item $A$ is part of some candidate key (not just superkey!)
        \end{itemize}
...
\begin{frame}{3NF Decomposition (4): Algorithm}
    \begin{itemize}
        ...
        \begin{itemize}
            \item a) Eliminate redundant FDs, resulting in a canonical cover $F_c$ of $F$
            \item b) Create a relation $R_i = XY$ for each FD $X \rightarrow Y$ in $F_c$
            \item c) If the key $K$ of $R$ does not occur in any relation $R_i$, create one more relation $R_i = K$
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{3NF Decomposition (7): Example}
    \begin{itemize}
        ...
        \begin{itemize}
            \item \texttt{cust\_banker\_branch = (customer\_id, employee\_id, branch\_name, type)}
        \end{itemize}
        \item The functional dependencies for this relation schema are:
        \begin{itemize}
            \item a) \texttt{customer\_id, employee\_id $\rightarrow$ branch\_name, type}
            \item b) \texttt{employee\_id $\rightarrow$ branch\_name}
            \item c) \texttt{customer\_id, branch\_name $\rightarrow$ employee\_id}
        \end{itemize}
        \item We first compute a canonical cover
        \begin{itemize}
            \item \texttt{branch\_name} is extraneous in the RHS of the $1^{st}$ dependency
            \item No other attribute is extraneous, so we get $F_c = $
            \begin{itemize}
...
\begin{frame}{3NF Decomposition (8): Example}
    \begin{itemize}
        ...
        \begin{itemize}
            \item \texttt{(customer\_id, employee\_id, type)}
            \item \texttt{(employee\_id, branch\_name)}
            \item \texttt{(customer\_id, branch\_name, employee\_id)}
        \end{itemize}
...
\begin{frame}{BCNF Decomposition: BCNF Definition}
    \begin{itemize}
        ...
        \begin{itemize}
            \item $\alpha \rightarrow \beta$ is trivial (that is, $\beta \subseteq \alpha$)
...
\begin{frame}{BCNF Decomposition (2): Testing for BCNF}
    \begin{itemize}
        ...
        \begin{itemize}
            \item a) Compute $\alpha^+$ (the attribute closure of $\alpha$), and
            \item b) Verify that it includes all attributes of $R$, that is, it is a superkey of $R$.
        \end{itemize}
        ...
        \begin{itemize}
            \item Consider $R = (A, B, C, D, E)$, with $F = \{A \rightarrow B, BC \rightarrow D\}$
...
\begin{frame}{BCNF Decomposition (3): Testing for BCNF Decomposition}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item $\triangleright$ for every set of attributes $\alpha \subseteq R_i$, check that $\alpha^+$ (the attribute closure of $\alpha$) either includes no attribute of $R_i - \alpha$, or includes all attributes of $R_i$.
                \item $\triangleright$ If the condition is violated by some $\alpha \rightarrow \beta$ in $F$, the dependency $\alpha \rightarrow (\alpha^+ - \alpha) \cap R_i$ can be shown to hold on $R_i$, and $R_i$ violates BCNF.
                \item $\triangleright$ We use above dependency to decompose $R_i$
            \end{itemize}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{BCNF Decomposition (4): Algorithm}
    \begin{itemize}
        ...
        \item b) If not, then
        \begin{itemize}
            \item Choose a dependency in $F^+$ that breaks the BCNF rules, say $A \rightarrow B$
            \item Create $R_1 = AB$
            \item Create $R_2 = (R - (B - A))$
            \item Note that: $R_1 \cap R_2 = A$ and $A \rightarrow AB \, (= R_1)$, so this is lossless decomposition
        \end{itemize}
        \item c) Repeat for $R_1$, and $R_2$
        \begin{itemize}
            \item By defining $F_1^+$ to be all dependencies in $F$ that contain only attributes in $R_1$
            \item Similarly $F_2^+$
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{BCNF Decomposition (7): Example}
    \begin{itemize}
        ...
        \begin{itemize}
            \item \texttt{course\_id $\rightarrow$ title, dept\_name, credits}
            \item \texttt{building, room\_number $\rightarrow$ capacity}
            \item \texttt{course\_id, sec\_id, semester, year $\rightarrow$ building, room\_number, time\_slot\_id}
        \end{itemize}
        \item A candidate key: \texttt{course\_id, sec\_id, semester, year}
        \item BCNF Decomposition:
        \begin{itemize}
            \item \texttt{course\_id $\rightarrow$ title, dept\_name, credits} holds
...
\begin{frame}{BCNF Decomposition (8): Example}
    \begin{itemize}
        ...
        \begin{itemize}
            \item \texttt{class-1(course\_id, sec\_id, semester, year, building, room\_number, capacity, time\_slot\_id)}
        \end{itemize}
