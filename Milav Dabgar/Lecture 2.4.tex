...
\begin{frame}{Module Objectives}
    \begin{itemize}
        \item To complete the understanding of basic query structure
    \end{itemize}
\end{frame}
...
\begin{frame}{Module Outline}
    \begin{itemize}
        ...
        \item Rename \texttt{AS} Operation
        \item String Values
        \item \texttt{Order By}
        \item \texttt{Select Top} / \texttt{Fetch}
        \item \texttt{Where} Clause Predicate
        \item Duplicates
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Examples}
    \begin{itemize}
        \item Find the names of all instructors who have taught some course and the course id
        \begin{lstlisting}[language=SQL]
select name, course_id 
from instructor, teaches 
where instructor.ID = teaches.ID;
        \end{lstlisting}
        \item Equi-Join, Natural Join
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Examples (2)}
    \begin{itemize}
        \item Find the names of all instructors in the Art department who have taught some course and the course id
        \begin{lstlisting}[language=SQL]
select name, course_id 
from instructor, teaches 
where instructor.ID = teaches.ID 
...
\begin{frame}[fragile]{Rename AS Operation}
    \begin{itemize}
        \item SQL allows renaming relations and attributes using the \texttt{as} clause: 
        \begin{center}\texttt{old\_name as new\_name}\end{center}
...
where T.salary > S.salary 
  and S.dept_name = 'Comp. Sci';
        \end{lstlisting}
        \item Keyword \texttt{as} is optional and may be omitted: \texttt{instructor as T} $\equiv$ \texttt{instructor T}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{String Operations}
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{percent} (\%): The \% character matches any substring
            \item \textbf{underscore} (\_): The \_ character matches any character
        \end{itemize}
...
where name like '%dar%';
        \end{lstlisting}
        \item Match the string '100\%': \texttt{like '100\textbackslash\%' escape '\textbackslash'}
        \item In that above we use backslash (\textbackslash) as the escape character
    \end{itemize}
\end{frame}
...
\begin{frame}{String Operations (2)}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \texttt{'\%Comp\%'} matches any string containing 'Comp' as a substring
            \item \texttt{'\_\_\_'} matches any string of exactly three characters
            \item \texttt{'\_\_\_\%'} matches any string of at least three characters
        \end{itemize}
        \item SQL supports a variety of string operations such as:
        \begin{itemize}
            \item concatenation (using \texttt{||})
            \item converting from upper to lower case (and vice versa)
...
order by name;
        ...
        \item Example: \texttt{order by name desc}
        \item Can sort on multiple attributes
        \item Example: \texttt{order by dept\_name, name}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Where Clause Predicates}
    \begin{itemize}
        ...
        \item Example: Find the names of all instructors with salary between \$90,000 and \$100,000 (that is, $\ge \$90,000$ and $\le \$100,000$)
        \begin{lstlisting}[language=SQL]
select name 
from instructor
