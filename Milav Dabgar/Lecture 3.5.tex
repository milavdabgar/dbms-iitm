...
\begin{frame}{Native Language $\leftrightarrow$ Query Language}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] \textbf{Embedded SQL}
                \item[$\triangleright$] \textbf{Dynamic SQL} (ODBC, JDBC)
            \end{itemize}
...
\begin{frame}{Functions and Procedures}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] Example: Functions to check if polygons overlap, or to compare images for similarity
            \end{itemize}
...
\begin{frame}{SQL Functions (2)}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] May contain multiple SQL statements between \texttt{begin} and \texttt{end}.
            \end{itemize}
...
\begin{frame}{Language Constructs for Procedures and Functions}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] May contain multiple SQL statements between \texttt{begin} and \texttt{end}.
                \item[$\triangleright$] Local variables can be declared within a compound statement.
            \end{itemize}
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{External Language Routines (2)*}
    \begin{block}{Pros and Cons}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] More efficient for many operations, and more expressive power
            \end{itemize}
            \item \textbf{Drawbacks}
            \begin{itemize}
                \item[$\triangleright$] Code to implement function may need to be loaded into database system and executed in the database system's address space.
...
\begin{frame}{External Language Routines (3)*: Security}
    \begin{block}{Security Approaches}
        \begin{itemize}[<+->]
            \item To deal with security problems, we can do one of the following:
            \begin{itemize}
                \item[$\triangleright$] \textbf{Use sandbox techniques}
                ...
                \item[$\triangleright$] \textbf{Run external language functions/procedures in a separate process}, with no access to the database process' memory
...
\begin{frame}{Trigger}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] To enforce data integrity rules via referential constraints and check constraints
                \item[$\triangleright$] To cause updates to other tables, automatically generate or transform values for inserted or updated rows, or invoke functions to perform tasks such as issuing alerts
            \end{itemize}
            \item To design a trigger mechanism, we must:
            \begin{itemize}
                \item[$\triangleright$] Specify the \textbf{events} (like \texttt{update}, \texttt{insert}, or \texttt{delete}) for the trigger to execute
                \item[$\triangleright$] Specify the \textbf{time} (\texttt{BEFORE} or \texttt{AFTER}) of execution
                \item[$\triangleright$] Specify the \textbf{actions} to be taken when the trigger executes
            \end{itemize}
...
\begin{frame}{Types of Triggers: BEFORE}
    \begin{block}{Types}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] Run \textbf{before} an \texttt{update}, or \texttt{insert}
                \item[$\triangleright$] Values that are being updated or inserted can be modified before the database is actually modified. You can use triggers that run before an update or insert to:
                \begin{itemize}
                    \item Check or modify values before they are actually updated or inserted in the database
                    \item Useful if user-view and internal database format differs
                    \item Run other non-database operations coded in user-defined functions
                \end{itemize}
            \end{itemize}
            \item \textbf{BEFORE DELETE triggers}
            \begin{itemize}
                \item[$\triangleright$] Run before a \texttt{delete}
                \begin{itemize}
                    \item Checks values (raises an error, if necessary)
                \end{itemize}
            \end{itemize}
        \end{itemize}
    \end{block}
...
\begin{frame}{Types of Triggers (2): AFTER}
    \begin{block}{Types}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] Run \textbf{after} an \texttt{update}, \texttt{insert}, or \texttt{delete}
                \item[$\triangleright$] You can use triggers that run after an update or insert to:
                \begin{itemize}
                    \item Update data in other tables
                    \item Useful for maintaining relationships between data or keeping an audit trail
                    \item Check against other data in the table or in other tables
                    \item Useful to ensure data integrity when referential integrity constraints aren't appropriate, or
                    \item when table check constraints limit checking to the current table only
                \end{itemize}
                \item[$\triangleright$] Run non-database operations coded in user-defined functions
                \begin{itemize}
                    \item Useful when issuing alerts or to update information outside the database
                \end{itemize}
            \end{itemize}
        \end{itemize}
    \end{block}
...
\begin{frame}{Row Level and Statement Level Triggers}
    \begin{block}{Trigger Levels}
        ...
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] Let Employee be a table with 100 rows. Suppose an update statement is executed to increase the salary of each employee by 10\%. Any row level update trigger configured on the table Employee will affect all the 100 rows in the table during this update.
            \end{itemize}
            ...
            \begin{itemize}
                \item[$\triangleright$] Used for each statement instead of for each row
                \item[$\triangleright$] Uses \texttt{referencing old table} or \texttt{referencing new table} to refer to temporary tables called transition tables containing the affected rows
                \item[$\triangleright$] Can be more efficient when dealing with SQL statements that update a large number of rows
            \end{itemize}
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}[fragile]{Triggering Events and Actions in SQL}
    \begin{block}{Overview}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] For example, \texttt{after update of grade on takes}
            \end{itemize}
            \item Values of attributes before and after an update can be referenced
            \begin{itemize}
                \item[$\triangleright$] \texttt{referencing old row as}: for deletes and updates
                \item[$\triangleright$] \texttt{referencing new row as}: for inserts and updates
            \end{itemize}
...
\begin{frame}[fragile]{Trigger to Maintain \texttt{credits\_earned} value}
    \begin{exampleblock}{Example}
        \begin{itemize}[<+->]
            \item Update student total credits upon grade update
            \begin{lstlisting}[language=SQL, basicstyle=\ttfamily\scriptsize]
...
\begin{frame}{How to use triggers?}
    \begin{block}{Usage Guidelines}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] Logging changes to a history table
                \item[$\triangleright$] Auditing users and their actions against sensitive tables
                \item[$\triangleright$] Adding additional values to a table that may not be available to an application (due to security restrictions or other limitations), such as:
                \begin{itemize}
                    \item Login/user name
                    \item Time an operation occurs
                    \item Server/database name
                \end{itemize}
                \item[$\triangleright$] Simple validation
            \end{itemize}
        \end{itemize}
    \end{block}
    \vspace{1em}
    \uncover<+->{\small{Source: \textit{SQL Server triggers: The good and the scary}}}
\end{frame}
...
\begin{frame}{How not to use triggers?}
    \begin{block}{Anti-patterns}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] triggers are used only as needed, and
                \item[$\triangleright$] to not allow them to become a one-size-fits-all solution for any data needs that happen to come along
            \end{itemize}
...
\begin{frame}{How not to use triggers? (2)}
    \begin{block}{Anti-patterns}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item[$\triangleright$] There are too many
                \item[$\triangleright$] Trigger code becomes complex
                \item[$\triangleright$] Triggers go cross-server - across databases over network
                \item[$\triangleright$] Triggers call triggers
                \item[$\triangleright$] Recursive triggers are set to ON. This database-level setting is set to off by default
                \item[$\triangleright$] Functions, stored procedures, or views are in triggers
                \item[$\triangleright$] Iteration occurs
            \end{itemize}
        \end{itemize}
    \end{block}
    \vspace{1em}
