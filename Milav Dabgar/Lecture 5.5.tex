...
\begin{frame}{Lossless Join Decomposition (4): Example}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Lossless-join decomposition: $R_1 \cap R_2 = \{B\}$ and $B \rightarrow BC$
            \item Dependency preserving
        \end{itemize}
        \item $R_1 = (A, B), R_2 = (A, C)$
        \begin{itemize}
            \item Lossless-join decomposition: $R_1 \cap R_2 = \{A\}$ and $A \rightarrow AB$
            \item Not dependency preserving (cannot check $B \rightarrow C$ without computing $R_1 \bowtie R_2$)
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Practice Problems on Lossless Join}
    \begin{itemize}
        ...
        \begin{itemize}
            \item i) $D_1 = R_1(AB), R_2(BC), R_3(ABDE), R_4(EG)$
            \item ii) $D_2 = R_1(ABC), R_2(ACDE), R_3(ADG)$
        \end{itemize}
        \item e) $R(ABCDEFGHIJ)$: $F = \{AB \rightarrow C, B \rightarrow F, D \rightarrow IJ, A \rightarrow DE, F \rightarrow GH\}$
        \begin{itemize}
            \item i) $D_1 = R_1(ABC), R_2(ADE), R_3(BF), R_4(FGH), R_5(DIJ)$
            \item ii) $D_2 = R_1(ABCDE), R_2(BFGH), R_3(DIJ)$
            \item iii) $D_3 = R_1(ABCD), R_2(DE), R_3(BF), R_4(FGH), R_5(DIJ)$
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Dependency Preservation (3): Example - Checking}
    ...
    \end{itemize}
    \vspace{1em}
    \textbf{Hence all dependencies are preserved.}
\end{frame}
...
\begin{frame}{Dependency Preservation (6): Example}
    \begin{itemize}
        ...
        \item Need to check for: ✭✭✭✭ $A \rightarrow BCD, A \rightarrow EF$, ✭✭✭✭ $BC \rightarrow AD, BC \rightarrow E, BC \rightarrow F$, ✘✘✘ $B \rightarrow F$, ✘✘✘ $D \rightarrow E$
        \item $(BC)^+ / F_1 = ABCD$. $(ABCD)^+ / F_2 = ABCDF$. $(ABCDF)^+ / F_3 = ABCDEF$. \textbf{Preserves} $BC \rightarrow E, BC \rightarrow F$
        \begin{itemize}
            \item $BC \rightarrow AD (R_1), AD \rightarrow E (R_3)$ implies $BC \rightarrow E$
            \item $B \rightarrow F (R_2)$ implies $BC \rightarrow F$
        \end{itemize}
        \item $(A)^+ / F_1 = ABCD$. $(ABCD)^+ / F_2 = ABCDF$. $(ABCDF)^+ / F_3 = ABCDEF$. \textbf{Preserves} $A \rightarrow EF$
        \begin{itemize}
            \item $A \rightarrow B (R_1), B \rightarrow F (R_2)$ implies $A \rightarrow F$
            \item $A \rightarrow D (R_1), D \rightarrow E (R_3)$ implies $A \rightarrow E$
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Dependency Preservation (7): Example}
    \begin{itemize}
        ...
        \begin{itemize}
            \item $B^+ / F = BF: B \rightarrow A$ cannot be inferred
            \item $C^+ / F = C: C \rightarrow A$ cannot be inferred
            \item $D^+ / F = DE: D \rightarrow A$ and $D \rightarrow BC$ cannot be inferred
            \item $A^+ / F = ABCDEF: A \rightarrow BC$ can be inferred, but it is equal to $A \rightarrow B$ and $A \rightarrow C$
            \item $F^+ / F = F: F \rightarrow B$ cannot be inferred
            \item $E^+ / F = E: E \rightarrow D$ cannot be inferred
        \end{itemize}
